...
class ValidationPipeline:
    ...
    async def process_pdf(self, pdf_path: str, max_workers: int = 5) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return self._create_summary(path.name, start_time, error=str(e), status="failed")
        ...
        logger.info(f"Pipeline {summary['status']} in {summary['duration_seconds']:.2f} seconds.")
    ...
    async def _run_validation(self, references: List[Paper], max_workers: int) -> List[Dict[str, Any]]:
        results = []
        semaphore = asyncio.Semaphore(max_workers)
        
        async def sem_task(index, paper):
            async with semaphore:
                res = await self._validate_single_reference(paper, index, len(references))
                return index, res
        ...
        for future in asyncio.as_completed(tasks):
            try:
                ...
                if result:
                    ...
                    self._notify_callbacks('on_reference_validation_end', references[index], result)
            except Exception as e:
                logger.error(f"Task error: {e}")
                
        return results

    async def _validate_single_reference(self, paper: Paper, index: int, total: int) -> Optional[Dict[str, Any]]:
        """
