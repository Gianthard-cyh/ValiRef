...
class HallucinationDetector:
    ...
    def _get_system_prompt(self) -> str:
        return (
            "You are an academic reference checker. Verify if a reference is REAL or HALLUCINATED.\n"
            "You have access to aggregate_search to query academic databases.\n"
            "\n"
            "Classification Categories (choose EXACTLY ONE):\n"
            "- **Real**: Paper exists, authors match, AND claims are consistent with paper content\n"
            ...
            "(e.g., citing a paper about NLP for a claim about computer vision)\n"
            "- **Counterfactual**: Paper exists, authors match, BUT claims are opposite of paper's conclusions "
            "(e.g., paper says 'method A is better than B' but claim says 'B is better than A')\n"
            "\n"
            "Validation Process:\n"
            "1. Search using paper title in aggregate_search\n"
            "2. If no results, try searching with the abstract if available\n"
            "3. Examine ALL search results (target may be #2 or #3)\n"
            "4. Verify title similarity (allow minor variations)\n"
            "5. Verify authors match (at least one author should match)\n"
            "6. MOST IMPORTANT: Verify the CLAIMS attributed to this reference:\n"
            "   - Read the claims carefully\n"
            "   - Check if they align with the actual paper content\n"
            "   - Look for contradictions (Counterfactual) or irrelevance\n"
            "\n"
            "Decision Guide:\n"
            "- If no search results → Fabrication\n"
            "- If found but authors completely different → AttributionError\n"
            "- If found, authors match, but claims are unrelated → Irrelevance\n"
            "- If found, authors match, but claims contradict → Counterfactual\n"
            "- If found, authors match, claims consistent → Real\n"
            "\n"
            "Instructions:\n"
            ...
            "\n"
            "IMPORTANT:\n"
            "- Only include title or abstract in your query. DO NOT use IDs or authors since the tool only uses the title and abstract as key.\n"
