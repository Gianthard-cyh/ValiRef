...
class PaperCrawler:
    ...
    def fetch_seeds(self, topic: str = "cs.AI", count: int = 10) -> List[Paper]:
        ...
        """
        logger.info(f"Starting seed sourcing from {self.source} for topic '{topic}', count={count}")
        ...
        if self.source == "arxiv":
            ...
            with Progress(
                ...
                for start in range(0, count, batch_size):
                    ...
                    batch_papers = self._fetch_from_arxiv(topic, current_batch_size, start=start)
                    
                    if not batch_papers:
                        logger.warning(f"No more papers found or error occurred at start={start}")
                        break
    ...
    def _fetch_from_arxiv(self, query: str, max_results: int, start: int = 0) -> List[Paper]:
        """
        从 arXiv API 获取数据。
