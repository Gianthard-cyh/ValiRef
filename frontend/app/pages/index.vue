<template>
  <div class="min-h-screen bg-white">
    ...
    <main class="max-w-7xl mx-auto px-4 py-6">
      <!-- Upload Section -->
      <div v-if="pageState === 'idle' || pageState === 'error'" class="max-w-xl mx-auto space-y-8">
      ...
      <!-- Processing Section -->
      <div v-if="pageState === 'processing'" class="max-w-xl mx-auto space-y-8">
        <div class="text-center space-y-2">
      ...
      <div v-if="pageState === 'completed' && currentResult" class="space-y-4">
        <!-- Top Actions -->
        ...
        <!-- Split View: PDF Left, Results Right -->
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 h-[calc(100vh-220px)] min-h-[500px]">
          <!-- Left: PDF Viewer -->
...
const { pageState, errorMessage, currentStatus, currentResult, progress, isProcessing } = storeToRefs(taskStore);
...
watch(currentResult, (result) => {
  if (result?.filename && selectedFile.value) {
...
async function submit() {
  ...
  try {
    ...
    await startPolling(response.task_id, response.filename);
  } catch (e: any) {
