...
def test_pipeline_pdf_processing():
    # Locate the test PDF
    pdf_path = Path(__file__).parent.parent / "data" / "11262_TamperTok_Forensics_Driv.pdf"
    ...
    real_count = sum(1 for r in results["results"] if r["validation"].get("is_hallucination") is False)
